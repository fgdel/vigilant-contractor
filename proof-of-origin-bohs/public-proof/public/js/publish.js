...
$(document).ready(function() {
  ...
  var refreshLatest = function(type, count, table) {
    $.getJSON('./api/latest/' + type + "/" + count, function(data) {
      var items = [];
      ...
      $.each(data, function(index, obj) {
        ...
        items.push('<tr><td>' + badge + '</td><td><a href="/details/' + obj.signature +
  ...
  var crypto_finish = function(hash) {
    ...
    //$.post('./api/v2/register/' + hash, onRegisterSuccess);
  };
  ...
  $( "#upload_form" ).submit(function( event ) { 
    ...
    $.ajax({
      ...
      beforeSend: function(){
        ...
        bar.width('0%');
        $('#description').remove();
        $('#description_container').append('<div id="wait"><h2> Registering Asset... Please wait...</h2><br/><img src="img/gears.gif"/></div>');
      },
        ...
        items.push(
            ...
            Object.keys(data).forEach(function(key) {
              if (key=="long_url" || key=="short_url")
                 ...
                 items.push('<tr><td>' + key+ '</td><td>' + data[key]+ ' [<a href="javascript:getItem(\''+data[key]+'\');">item</a>]</td></tr>');
              else
      ...
      error: function (e) {
        ...
        $("#description_container").text(e.responseText);
        console.log("ERROR : ", e);
        $("#btnSubmit").prop("disabled", false);
 ...
 $( "#upload_forrm" ).submit(function( event ) {
...
//  $.post('./api/v2/publish/' + signature,{name: name, email: email, message: message }, function(data){
  $.post('./api/v2/publish/' + {name: name, email: email, message: message }, function(data){
      console.log(data);
    $('#wait').remove();
    $('#description_container').append("<h2>Success</h2>");
    var items = [];
